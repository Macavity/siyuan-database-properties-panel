<script lang="ts">
    ...
    import MAssetValue from './MAssetValue.svelte';
    import DateValue from './DateValue.svelte';
    import RelationValue from './RelationValue.svelte';
    import SelectChip from "@/components/SelectChip.svelte";
    import AttributeViewRollup from "@/components/AttributeViewRollup.svelte";
...
{:else if value.type === 'text'}
    <textarea style="resize: vertical" rows="{value.text.content.split('\n').length}"
...
{:else if value.type === 'created' || value.type === 'updated'}
    <span data-content={value[value.type].content}>{dayjs(value[value.type].content).format("YYYY-MM-DD HH:mm")}</span>
{:else if value.type === 'url'}
    <input value="{value.url.content}" class="b3-text-field b3-text-field--text fn__flex-1">
    <span class="fn__space"></span>
    <a href={value.url.content} target="_blank" aria-label={window.siyuan.languages.openBy}
...
{:else if value.type === 'phone'}
    <input value={value.phone.content} class="b3-text-field b3-text-field--text fn__flex-1">
    <span class="fn__space"></span>
    <a href="tel:{value.phone.content}" target="_blank" aria-label={window.siyuan.languages.openBy}
       class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips">
        <svg>
...
{:else if value.type === 'template'}
    <div class="fn__flex-1">{value.template.content}</div>
{:else if value.type === 'email'}
    <input value={value.email.content} class="b3-text-field b3-text-field--text fn__flex-1">
    <span class="fn__space"></span>
    <a href="mailto:{value.email.content}" target="_blank" aria-label="{window.siyuan.languages.openBy}"
       class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips">
        <svg>
