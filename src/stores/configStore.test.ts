...
import { useConfigStore, defaultConfig, createConfigFromStorage } from "./configStore";
...
describe("configStore", () => {
  ...
  describe("column visibility", () => {
    ...
    test("setColumnVisibility shows column by removing entry", () => {
      ...
      expect(configStore.columnVisibility[avId]).toBeUndefined();
    });
  ...
  describe("createConfigFromStorage", () => {
    test("creates config with defaults for missing properties", () => {
      const partial = { showPrimaryKey: true };
  ...
  describe("setSetting", () => {
    test("updates boolean settings", () => {
      ...
      expect(configStore.showPrimaryKey).toBe(true);
    });

    test("updates showEmptyAttributes", () => {
      configStore.setSetting("showEmptyAttributes", true);
      expect(configStore.showEmptyAttributes).toBe(true);
    });
  });
  ...
  describe("getSettingsObject", () => {
    test("returns settings without loading flag", () => {
      ...
      expect(settings.columnVisibility[avId]).toEqual({ [columnId]: false });
    });
  });
});
